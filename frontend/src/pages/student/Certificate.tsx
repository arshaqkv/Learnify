...
const CertificatePDF = ({ user, course }: { user: any; course: any }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.footer}>
        <View>
          ...
          >{`${course?.creator?.firstname} ${course?.creator?.lastname}`}</Text>
        </View>
        <View>
          <View style={styles.line}></View>
  ...
  return (
    <div className="flex flex-col items-center my-3 bg-gray-50">
      <div className="p-12 shadow-2xl rounded-lg w-[850px] text-center border-8 border-gray-300">
        ...
        <h2 className="text-4xl font-bold text-blue-700 mt-6">
          Certificate of Completion
        </h2>
        <p className="text-lg mt-6 text-gray-700 italic">
      ...
      >
        {({ loading }) => (
          <Button
            variant={"outline"}
            className=" px-6 py-2 rounded-lg shadow-lg hover:bg-blue-800"
          >
